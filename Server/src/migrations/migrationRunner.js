...
import { UserMigration, rollback as userRollback } from "./001_migrate_user_collection.js";
import { MDAMigration, rollback as mdaRollback } from "./002_migrate_mda_collection.js";
import { AdminMigration, rollback as adminRollback } from "./003_migrate_admin_collection.js";
...
class MigrationRunner {
  ...
  async ensureConnection() {
    if (mongoose.connection.readyState === 0) {
      this.log("Database not connected. Please ensure database connection before running migrations.", 'error');
  ...
  async initializeMigrationTracking() {
    ...
    await migrationCollection.createIndex({ migration_name: 1 }, { unique: true });
  ...
  async runAllMigrations() {
    ...
    try {
      ...
      for (const migration of this.migrations) {
        ...
        if (isCompleted) {
          this.log(`Migration ${migration.name} already completed, skipping...`);
          results.push({
            name: migration.name,
            success: true,
            skipped: true,
        ...
        this.log(`=== Completed ${migration.name} in ${result.duration}ms ===\n`);
      }
      ...
      const totalDuration = Date.now() - startTime;
      this.log(`Migration process failed after ${totalDuration}ms: ${error.message}`, 'error');
  ...
  async validateAllMigrations() {
    ...
    // Overall validation
    const allValid = userValidation.valid && mdaValidation.valid && adminValidation.valid;
  ...
  async validateUserMigration(db) {
    ...
    const withUsername = await collection.countDocuments({ username: { $exists: true } });
    const withContactEmail = await collection.countDocuments({ contactEmail: { $exists: true } });
    const withMdaReference = await collection.countDocuments({ mdaReference: { $exists: true } });
    const withOldEmail = await collection.countDocuments({ email: { $exists: true } });
    const withOldMdaId = await collection.countDocuments({ mdaId: { $exists: true } });
    
    const valid = total === withUsername && 
  ...
  async validateMDAMigration(db) {
    ...
    const total = await collection.countDocuments({});
    const withReports = await collection.countDocuments({ reports: { $exists: true, $type: "array" } });
    const withOldReportUrl = await collection.countDocuments({ reportUrl: { $exists: true } });
  ...
  async validateAdminMigration(db) {
    ...
    const total = await collection.countDocuments({});
    const withCanBeDeleted = await collection.countDocuments({ canBeDeleted: { $exists: true } });
    const superAdmins = await collection.countDocuments({ role: 'superadmin', canBeDeleted: false });
    ...
    try {
      ...
      for (let i = this.migrations.length - 1; i >= 0; i--) {
        ...
        try {
          ...
          if (!result.success) {
            this.log(`Rollback ${migration.name} failed: ${result.error}`, 'error');
          } else {
            this.log(`=== Rollback ${migration.name} completed ===\n`);
          }
          
        } catch (error) {
          this.log(`Rollback ${migration.name} failed: ${error.message}`, 'error');
          results.push({
            name: migration.name,
            success: false,
            error: error.message
  ...
  async getMigrationStatus() {
    try {
      ...
      for (const migration of this.migrations) {
        const record = completedMigrations.find(m => m.migration_name === migration.name);
