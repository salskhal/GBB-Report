...
// Define props for the admin dashboard layout
...
export default function AdminDashboardLayout({
  ...
  const navItems: NavItem[] = [
    {
      ...
      label: "Overview",
      icon: <Home size={20} />,
      exact: true, // This will ensure it's only active when the path is exactly "/admin/dashboard"
    },
    {
  ...
  const getAdminRole = (): string => {
    return admin?.role || "System Administrator";
  };
        ...
        <div className="p-4">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-full bg-white text-primary flex items-center justify-center">
              {adminAvatar ? (
                <img src={adminAvatar} alt="Admin" className="rounded-full" />
              ) : (
                <User size={20} />
              )}
            </div>
            <div className="flex-grow">
              ...
              <p className="text-xs ">{getAdminRole()}</p>
            </div>
          </div>
        </div>
      </aside>
        ...
        <footer className="bg-white border-t border-gray-200 py-4 px-6">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600">
              &copy; {new Date().getFullYear()} Galaxy Backbone Admin Panel. All rights
              reserved.
            </p>
          </div>
        </footer>
